...
* `_process_frames`: An intermediate processing step on the single channel* audio stream before mixing. This is called from the `AudioServer` thread. Parameters are:
...
* `_mix_channel`: If your spatialization requires non-linear effects happening on per-channel basis (i.e. something that could not be achieved by using per-channel volumes modulating a single source audio stream), this method can be used to do custom mixing per channel.  This is called from the `AudioServer` thread. Parameters are similar to `_process_frames`:
    ...
    * `output_buf`: The output buffer to write the processed audio stream into. This buffer is **cumulative**, and should have mixed stream _added_ to the output, not overwritten.
    * `source_buf`: The input buffer to read the incoming audio stream info from.
    * `frame_count`: The length of `source_buf` and `output_buf`; i.e. the number of `AudioFrame`s to process.
...
**Note:** you should not use member variables on the `AudioSpatializerInstance` to share data between `_calculate_spatializion`, `_process_frames`, and `_mix_channel`. These methods are running on different threads, and doing so can result in race conditions. Add any important data to `SpatializerParameters`, and use them in the process/mix methods.
...
When custom mix channels is enabled, `AudioSpatializerInstance` creates up to four proxy `AudioStreamPlayback`s, one for each audio channel. When mixing, the `_mix_channel` method is called to mix and accumulate each internal playback stream into the combined mix buffers, one for each channel (using the `mix_volumes` spatial parameter). This can result in up to four internal mix buffers (depending on the number of channels of the audio driver), which are then passed to the `AudioServer` via the proxy `AudioStreamPlayback`s.
